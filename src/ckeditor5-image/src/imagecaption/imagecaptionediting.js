...
export default class ImageCaptionEditing extends Plugin {
    ...
    _updateCaptionVisibility(viewWriter) {
        ...
        if (viewCaption) {
            ...
            // Was any caption selected before?
            if (lastCaption) {
                const viewModified = hideCaptionIfEmpty(
                    lastCaption,
                    viewWriter,
                );
        ...
        for (const entry of changes) {
            if (entry.type == 'insert' && entry.name != '$text') {
                ...
                if (!item.is('image') && item.childCount) {
                    for (const nestedItem of model
                        .createRangeIn(item)
                        .getItems()) {
                        if (
                            nestedItem.is('image') &&
                            !getCaptionFromImage(nestedItem)
                        ) {
...
function insertViewCaptionAndBind(
    ...
    const viewPosition = conversionApi.writer.createPositionAt(
        viewImage,
        'end',
    );
